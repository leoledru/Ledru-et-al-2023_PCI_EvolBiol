%% Initialisation Océane pour code avec perturbations

%D= coefficient de diffusion est devenu Df (pr commu tondeuse ou af)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for i = 1:length(prep)
    ...
    % Soit commu foraging soit tondeuse
        prep{i}.Df = N(i) ; %pas de foraging, tondeuse
    ...
    A_foraging = prep{i}.Df*A_foraging/(dx^2) ;

    prep{i}.pre_perturb.A_foraging = A_foraging ;
...
prep{i}.pre_perturb.effort_dynamic = effort_dynamic ;

animal_density = animal_density.*(animal_density>seuil_abondance);
prep{i}.pre_perturb.animal_density = animal_density ;
plant_density = plant_density.*(plant_density>seuil_abondance);
prep{i}.pre_perturb.plant_density = plant_density ;

prep{i}.pre_perturb.t_pre = t;

%ISOLATE LAST TIME LAPS to initialize perturbation 
 animal_density_last_step = animal_density(end,:) ;
 prep{i}.pre_perturb.animal_density_last_step = animal_density_last_step' ;
 plant_density_last_step = plant_density(end,:) ;
 prep{i}.pre_perturb.plant_density_last_step = plant_density_last_step' ;
 effort_ij0_last_step = effort_dynamic(end,:);
 prep{i}.pre_perturb.effort_ij0_last_step = effort_ij0_last_step';
 
end

save pre_perturb.mat  A_animal A_plant animal_intrinsic_growth... 
                        Compet K sigmaC sigmaK ; %save var qui vont être ecrasees au cours des perturb
save prep.mat  prep %prep contient les sorties qui nous interessent

save initialisation.mat  -v7.3 %save tout pour verifier si pb