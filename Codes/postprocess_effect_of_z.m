%% POSTPROCESS POUR LES FIGURES DE L'EFFET DE Z SUR BIOMASSE, DIV, PRODUCTIVITY
...
BIOMASS_R = zeros(NjK,number_of_foraging);
BIOMASS_C = zeros(NjK,number_of_foraging);
FDIS_C    = zeros(NjK,number_of_foraging);
FDIS_R    = zeros(NjK,number_of_foraging);
PROD      = zeros(NjK,number_of_foraging);
...
for jK = 1:NjK
    ...
    for ii = 1:size(postprocess,2)
        ...
        animal = Out_A{:,ii};
        for jj = 1:11
            animal_niche = [animal_niche,sum(animal(:,jj:number_of_animals:end),2)];
            %         (jj-1)*number_of_foraging+1:jj*number_of_foraging),2)];
            
            %         animal_niche = [animal_niche,sum(animal(:,(jj-1)*number_of_foraging+1:jj*number_of_foraging),2)];
        end
        Out_a{:,ii} = animal_niche;
    end
    
    % for ii = 1:size(postprocess,2)
    %     animal = posprocess{1,ii};
    %     for jj = 1:number_of_foraging
    %         animal_niche(jj,:) = sum(animal((jj-1)*number_of_animals+1:jj*number_of_animals,:),2)';
    %     end
    %     Out_a{:,ii} = animal_niche;
    % end
    ...
    % % plot(1:101,biomass_animal(:,1:10:end),'LineWidth',3)
    %
    % xlim([0 101])
    ...
    BIOMASS_C(jK,:) = mean(final_biomass_a,2);
    final_biomass_p = biomass_plant(:,end-10:end); % average over last 100 steps
    BIOMASS_R(jK,:) = mean(final_biomass_p,2);
    ...
    FDIS_C(jK,:) = mean(FDis_animal(end-10:end,:),1);
    FDIS_R(jK,:) = mean(FDis_plant(end-10:end,:),1);
    ...
    PROD(jK,:) =  mean(Productivity_f(:,end-10:end),2);
end
%
% %% BOXPLOT
...
%% FIGURE
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% BIOMMASS RESOURCE AND CONSUMER
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Gaussian kernel
figure(11)
clf
% superpose curves
yyaxis left
scatter(foraging_trait,BIOMASS_C(1,:),50,'filled')
ylabel('Biomass of consumers $C$','interpreter','latex','FontSize',20)
hold on
yyaxis right
ylabel('Biomass of resources $R$','interpreter','latex','FontSize',20)
scatter(foraging_trait,BIOMASS_R(1,:),'s','filled','MarkerFaceColor',Color(5,:))
legend('consumers','resources','interpreter','latex','FontSize',15,'Location','East')
xlabel('foraging trait','interpreter','latex','FontSize',20)
ax = gca;
...
% superpose curves
yyaxis left
scatter(foraging_trait,BIOMASS_C(2,:),50,'filled')
ylabel('Biomass of consumers $C$','interpreter','latex','FontSize',20)
hold on
yyaxis right
ylabel('Biomass of resources $R$','interpreter','latex','FontSize',20)
scatter(foraging_trait,BIOMASS_R(2,:),'s','filled','MarkerFaceColor',Color(5,:))
legend('consumers','resources','interpreter','latex','FontSize',15,'Location','East')
xlabel('foraging trait','interpreter','latex','FontSize',20)
ax = gca;
ax.YAxis(1).Color = Color(1,:);
ax.YAxis(2).Color = Color(5,:); %[.2 .8 .2];
hold off
%% Compare Gaussian Quartic
figure(10)
clf
% superpose curves
yyaxis left
s1 = scatter(foraging_trait,BIOMASS_C(1,:),50,'filled')
hold on
% plot(foraging_trait,BIOMASS_C(2,:),'--','LineWidth',4,'color',Color(1,:))
plot(foraging_trait,BIOMASS_C(3,:),'-.','LineWidth',4,'color',Color(1,:))
ylabel('Biomass of consumers $C$','interpreter','latex','FontSize',20)

yyaxis right
ylabel('Biomass of resources $R$','interpreter','latex','FontSize',20)
s2 = scatter(foraging_trait,BIOMASS_R(1,:),50,'s','filled','MarkerFaceColor',Color(5,:))
hold on
% plot(foraging_trait,BIOMASS_R(2,:),'--','LineWidth',4,'color',Color(5,:))
plot(foraging_trait,BIOMASS_R(3,:),'-.','LineWidth',4,'color',Color(5,:))

legend([s1,s2],{'consumers','resources'},'interpreter','latex','FontSize',15,'Location','East')
xlabel('foraging trait','interpreter','latex','FontSize',20)
ax = gca;
ax.YAxis(1).Color = Color(1,:);
ax.YAxis(2).Color = Color(5,:); %[.2 .8 .2];
hold off
%% FDIS Resource and consumer
%% Figures FDis
% figure
% clf
% yyaxis left
...
% ylabel({'Functional diversity of';'consumers $FDis_C$'},'interpreter','latex','FontSize',20)
%
% % figure
% % clf
% yyaxis right
...
% hold off
% xlabel('Foraging trait $z$','interpreter','latex','FontSize',20)
% ylabel({'Functional diversity of'; 'resources $FDis_R$'},'interpreter','latex','FontSize',20)
%% Compare Gaussian Quartic
figure(20)
clf
% superpose curves
yyaxis left
s1 = scatter(foraging_trait,FDIS_C(1,:),50,'filled')
hold on
% plot(foraging_trait,FDIS_C(2,:),'--','LineWidth',4,'color',Color(1,:))
plot(foraging_trait,FDIS_C(3,:),'-.','LineWidth',4,'color',Color(1,:))
ylabel({'Functional diversity of'; 'consumers $FDis_C$'},'interpreter','latex','FontSize',20)

yyaxis right
ylabel({'Functional diversity of'; 'resources $FDis_R$'},'interpreter','latex','FontSize',20)
s2 = scatter(foraging_trait,FDIS_R(1,:),50,'s','filled','MarkerFaceColor',Color(5,:))
hold on
% plot(foraging_trait,FDIS_R(2,:),'--','LineWidth',4,'color',Color(5,:))
plot(foraging_trait,FDIS_R(3,:),'-.','LineWidth',4,'color',Color(5,:))
legend([s1,s2],{'consumers','resources'},'interpreter','latex','FontSize',15,'Location','East')
xlabel('foraging trait','interpreter','latex','FontSize',20)
ax = gca;
ax.YAxis(1).Color = Color(1,:);
...
% superpose curves
yyaxis left
scatter(foraging_trait,FDIS_C(1,:),50,'filled')
ylabel({'Functional diversity of'; 'consumers $FDis_C$'},'interpreter','latex','FontSize',20)
hold on
yyaxis right
ylabel({'Functional diversity of'; 'resources $FDis_R$'},'interpreter','latex','FontSize',20)
scatter(foraging_trait,FDIS_R(1,:),'s','filled','MarkerFaceColor',Color(5,:))
legend('consumers','resources','interpreter','latex','FontSize',15,'Location','East')
xlabel('foraging trait','interpreter','latex','FontSize',20)
ax = gca;
ax.YAxis(1).Color = Color(1,:);
...
% superpose curves
yyaxis left
scatter(foraging_trait,FDIS_C(2,:),50,'filled')
ylabel({'Functional diversity of'; 'consumers $FDis_C$'},'interpreter','latex','FontSize',20)
hold on
yyaxis right
ylabel({'Functional diversity of'; 'resources $FDis_R$'},'interpreter','latex','FontSize',20)
scatter(foraging_trait,FDIS_R(2,:),'s','filled','MarkerFaceColor',Color(5,:))
legend('consumers','resources','interpreter','latex','FontSize',15,'Location','East')
xlabel('foraging trait','interpreter','latex','FontSize',20)
ax = gca;
ax.YAxis(1).Color = Color(1,:);
...
scatter(foraging_trait,PROD(1,:),50,'filled')
hold on
% plot(foraging_trait,PROD(2,:),'--','LineWidth',4,'color',Color(1,:))
plot(foraging_trait,PROD(3,:),'-.','LineWidth',4,'color',Color(1,:))
% plot(x,v,'k','LineWidth',3)
hold off
xlabel('Foraging trait $z$','interpreter','latex','FontSize',20)
ylabel('Productivity $Prod$','interpreter','latex','FontSize',20)
...
figure(32)
clf
scatter(foraging_trait,PROD(2,:),50,'filled')
hold on
hold off
xlabel('Foraging trait $z$','interpreter','latex','FontSize',20)
ylabel('Productivity $Prod$','interpreter','latex','FontSize',20)
...
% %%% Figure Diff = Prod(z)-Prod(z=0)
% Diff_Prod = productivity-productivity(1);
% 
% figure
% clf
% scatter(foraging_trait,Diff_Prod,50,'filled')
% hold on
% % plot(x,v,'k','LineWidth',3)
% hold off
% xlabel('Foraging trait $z$','interpreter','latex','FontSize',20)
% ylabel('Difference of productivity $\Delta Prod$','interpreter','latex','FontSize',20)
